...
while( <LIST_FILE> ) {
    ...
    my $url = "http://localhost/jcr13/ticketServer/server.php?action=sell_ticket&security_data=$ARGV[2]&email=$email&name=$name&reference=bulk_manual&tags=$ARGV[1]&security_hash=$ARGV[3]";
    print "  url = $url\n";
