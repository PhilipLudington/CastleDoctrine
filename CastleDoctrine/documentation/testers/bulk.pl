...
while( <LIST_FILE> ) {
    ...
    my $url = "http://thecastledoctrine.net/ts/server.php?action=sell_ticket&security_data=$ARGV[2]&email=$email&name=$name&reference=manual&tags=$ARGV[1]&security_hash=$ARGV[3]";
    print "  url = $url\n";
